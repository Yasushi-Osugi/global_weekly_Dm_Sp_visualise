...
# ****************************
def connect_dict2tree_In4Dm(node, node_psi_dict):

    print('sconnect_dict2tree_In4Dm node.name',node.name)

    node.psi4demand = node_psi_dict.get(node.name)

    for child in node.children:

        connect_dict2tree_In4Dm(child, node_psi_dict)



def connect_dict2tree_In4Sp(node, node_psi_dict):

    node.psi4supply = node_psi_dict.get(node.name)

    for child in node.children:

        connect_dict2tree_In4Sp(child, node_psi_dict)
...
# 4. node_psi_dict=[node1: psi_list1,,,]を生成、treeのnode.psi4demandに接続する
    ...
    # *********************************
    node_psi_dict_In4Dm = {} # node_psi辞書を定義
    node_psi_dict_In4Sp = {} # node_psi辞書を定義
    ...
    #rootからtree nodeをinbound4demand=preorder順に検索 node_psi辞書をnodeにset
    connect_dict2tree_In4Dm(root_node_inbound, node_psi_dict_In4Dm)
    connect_dict2tree_In4Sp(root_node_inbound, node_psi_dict_In4Sp)
